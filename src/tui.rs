...
impl App {
    ...
    fn load_case(&mut self, filename: &str) {
        ...
        net.rebuild_bus_map();
        self.status_msg = format!("Loaded: {} ({} buses, {} branches)", filename, net.buses.len(), net.branches.len());
        ...
        if let Some(net) = &mut self.network {
            if net.dc_approximation() {
                self.status_msg = format!(
                    ...
                    net.buses.len(), net.branches.len()
                );
            } else {
    ...
    fn toggle_status(&mut self) {
        ...
        match self.active_tab {
            ...
            Tab::Branches => {
                if row >= net.branches.len() { return; }
                let br = &mut net.branches[row];
                br.branch_status = !br.branch_status;
                self.status_msg = format!("Branch {} {}", br.id,
                    if br.branch_status { "in service" } else { "out of service" });
            }
            _ => {
    ...
    fn start_edit(&mut self) {
        ...
        self.editing = Some(EditState { row, col: 0, buf: val, col_count });
        self.mode = Mode::Editing;
        self.status_msg = String::from("Editing: Left/Right to select field, type to modify, Enter to confirm, Esc to cancel");
    }
    ...
    fn cell_value(&self, row: usize, col: usize) -> String {
        ...
        match self.active_tab {
            Tab::Buses => {
                if row >= net.buses.len() { return String::new(); }
                let b = &net.buses[row];
                match col {
                    ...
                    6 => { let (p, _) = net.bus_mismatch(b.bus_id); format!("{:.2}", p) },
                    7 => { let (_, q) = net.bus_mismatch(b.bus_id); format!("{:.2}", q) },
                    8 => if b.bus_status { "1".into() } else { "0".into() },
                    _ => String::new(),
                }
            }
            Tab::Branches => {
                if row >= net.branches.len() { return String::new(); }
                ...
                match col {
                    ...
                    7 => format!("{:.3}", br.flow),
                    8 => if br.branch_status { "1".into() } else { "0".into() },
                    _ => String::new(),
                }
            }
            Tab::Generators => {
                if row >= net.generators.len() { return String::new(); }
                ...
                match col {
                    ...
                    6 => format!("{:.3}", g.p_min),
                    7 => format!("{:.3}", g.p_max),
                    8 => if g.gen_status { "1".into() } else { "0".into() },
                    _ => String::new(),
                }
            }
            Tab::Loads => {
                if row >= net.loads.len() { return String::new(); }
                let l = &net.loads[row];
                match col {
    ...
    fn apply_edit(&mut self) {
        ...
        match self.active_tab {
            Tab::Buses => {
                ...
                match col {
                    1 => b.bus_name = val,
                    3 => if let Ok(v) = val.parse::<f32>() { b.voltage = v; },
                    4 => if let Ok(v) = val.parse::<f32>() { b.angle = v; },
                    5 => if let Ok(v) = val.parse::<f32>() { b.nom_voltage = v; },
                    8 => {
            ...
            Tab::Branches => {
                ...
                let br = &mut net.branches[row];
                match col {
                    1 => if let Ok(v) = val.parse::<usize>() { br.from_bus = v; },
                    2 => if let Ok(v) = val.parse::<usize>() { br.to_bus = v; },
                    4 => if let Ok(v) = val.parse::<f32>() { br.resistance = v; },
                    5 => if let Ok(v) = val.parse::<f32>() { br.reactance = v; },
                    6 => if let Ok(v) = val.parse::<f32>() { br.operating_limit = v; },
                    8 => br.branch_status = val != "0",
                    _ => {}
                }
            }
            Tab::Generators => {
                if row >= net.generators.len() { return; }
                let g = &mut net.generators[row];
                match col {
                    1 => if let Ok(v) = val.parse::<usize>() { g.gen_bus_id = v; },
                    2 => g.gen_name = val,
                    3 => if let Ok(v) = val.parse::<f32>() { g.p_gen = v; },
                    4 => if let Ok(v) = val.parse::<f32>() { g.q_gen = v; },
                    5 => if let Ok(v) = val.parse::<f32>() { g.v_setpoint = v; },
                    6 => if let Ok(v) = val.parse::<f32>() { g.p_min = v; },
                    7 => if let Ok(v) = val.parse::<f32>() { g.p_max = v; },
                    8 => g.gen_status = val != "0",
                    _ => {}
                }
            }
            Tab::Loads => {
                if row >= net.loads.len() { return; }
                let l = &mut net.loads[row];
                match col {
                    1 => if let Ok(v) = val.parse::<usize>() { l.bus_id = v; },
                    2 => l.load_name = val,
                    3 => if let Ok(v) = val.parse::<f32>() { l.real_load = v; },
                    4 => if let Ok(v) = val.parse::<f32>() { l.imag_load = v; },
                    _ => {}
                }
            }
        }
    ...
    fn col_label(&self, col: usize) -> &'static str {
        match self.active_tab {
            Tab::Buses => match col {
                0 => "ID", 1 => "Name", 2 => "Type", 3 => "Voltage",
                4 => "Angle", 5 => "Nom kV", 6 => "P mis", 7 => "Q mis", 8 => "Status", _ => "",
            },
            Tab::Branches => match col {
                0 => "ID", 1 => "From", 2 => "To", 3 => "Type",
                4 => "R", 5 => "X", 6 => "RateA", 7 => "Flow", 8 => "Status", _ => "",
            },
            Tab::Generators => match col {
                0 => "ID", 1 => "Bus", 2 => "Name", 3 => "P",
                4 => "Q", 5 => "Vset", 6 => "Pmin", 7 => "Pmax", 8 => "Status", _ => "",
            },
            Tab::Loads => match col {
                0 => "ID", 1 => "Bus", 2 => "Name", 3 => "P", 4 => "Q", _ => "",
            },
        }
    }
}
            ...
            if let Event::Key(key) = event::read()? {
                ...
                match app.mode {
                    Mode::Normal => match key.code {
                        ...
                        KeyCode::Char('b') => {
                            if app.active_tab == Tab::Buses && app.network.is_some() {
...
fn draw_ui(f: &mut Frame, app: &mut App) {
    ...
    // Title bar
    let case_name = app.network.as_ref()
...
fn draw_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    match app.active_tab {
        Tab::Buses => {
            let header = Row::new(vec!["ID", "Name", "Type", "Voltage", "Angle", "Nom kV", "P mis", "Q mis", "Status"])
                .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))
                .bottom_margin(1);
            let rows: Vec<Row> = net.buses.iter().enumerate().map(|(i, b)| {
            ...
            let table = Table::new(rows, [
                Constraint::Length(6), Constraint::Length(16), Constraint::Length(5),
                Constraint::Length(12), Constraint::Length(10), Constraint::Length(10),
                Constraint::Length(10), Constraint::Length(10), Constraint::Length(6),
            ])
            .header(header)
            .block(Block::default().borders(Borders::ALL).title(format!(" Buses ({}) ", net.buses.len())))
            .row_highlight_style(Style::default().bg(Color::DarkGray));
            f.render_stateful_widget(table, area, &mut app.table_state);
        }
        Tab::Branches => {
            let header = Row::new(vec!["ID", "From", "To", "Type", "R", "X", "RateA", "Flow", "Status"])
                .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))
                .bottom_margin(1);
            let rows: Vec<Row> = net.branches.iter().enumerate().map(|(i, br)| {
                let cells = vec![
            ...
            let table = Table::new(rows, [
                Constraint::Length(6), Constraint::Length(8), Constraint::Length(8), Constraint::Length(5),
                Constraint::Length(12), Constraint::Length(12), Constraint::Length(10),
                Constraint::Length(12), Constraint::Length(6),
            ])
            .header(header)
            .block(Block::default().borders(Borders::ALL).title(format!(" Branches ({}) ", net.branches.len())))
            .row_highlight_style(Style::default().bg(Color::DarkGray));
            f.render_stateful_widget(table, area, &mut app.table_state);
        }
        Tab::Generators => {
            let header = Row::new(vec!["ID", "Bus", "Name", "P", "Q", "Vset", "Pmin", "Pmax", "Status"])
                .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))
                .bottom_margin(1);
            let rows: Vec<Row> = net.generators.iter().enumerate().map(|(i, g)| {
                let cells = vec![
            ...
            let table = Table::new(rows, [
                Constraint::Length(6), Constraint::Length(6), Constraint::Length(18), Constraint::Length(10),
                Constraint::Length(10), Constraint::Length(10), Constraint::Length(10), Constraint::Length(10), Constraint::Length(6),
            ])
            .header(header)
            .block(Block::default().borders(Borders::ALL).title(format!(" Generators ({}) ", net.generators.len())))
            .row_highlight_style(Style::default().bg(Color::DarkGray));
            f.render_stateful_widget(table, area, &mut app.table_state);
        }
        Tab::Loads => {
            let header = Row::new(vec!["ID", "Bus", "Name", "P (MW)", "Q (MVAR)"])
                .style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))
                .bottom_margin(1);
            let rows: Vec<Row> = net.loads.iter().enumerate().map(|(i, l)| {
                let cells = vec![
            ...
            let table = Table::new(rows, [
                Constraint::Length(6), Constraint::Length(8), Constraint::Length(20),
                Constraint::Length(12), Constraint::Length(12),
            ])
            .header(header)
            .block(Block::default().borders(Borders::ALL).title(format!(" Loads ({}) ", net.loads.len())))
            .row_highlight_style(Style::default().bg(Color::DarkGray));
            f.render_stateful_widget(table, area, &mut app.table_state);
        }
    }
    ...
    if app.mode == Mode::Editing {
        if let Some(ref edit) = app.editing {
            ...
            let p = Paragraph::new(text)
                ...
                .block(Block::default().borders(Borders::ALL).title(" Edit Field (Left/Right to switch) ").border_style(Style::default().fg(Color::Yellow)));
...
fn draw_file_browser(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let items: Vec<ListItem> = app.file_list.iter().map(|name| {
...
fn draw_bus_display(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    lines.push(Line::from(vec![
        Span::styled("Bus ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(bus.bus_id.to_string(), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        Span::raw("  "),
    ...
    // Voltage and angle
    let status_color = if bus.bus_status { Color::Green } else { Color::Red };
    lines.push(Line::from(vec![
        Span::styled("Status: ", Style::default().fg(Color::DarkGray)),
        Span::styled(if bus.bus_status { "In Service" } else { "Out of Service" }, Style::default().fg(status_color)),
        Span::raw("    "),
        Span::styled("Type: ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{}", bus.bus_type), Style::default().fg(Color::White)),
    ]));
    lines.push(Line::from(vec![
        Span::styled("Voltage: ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{:.6} pu", bus.voltage), Style::default().fg(Color::White)),
        Span::raw("    "),
        Span::styled("Angle: ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{:.4} deg", bus.angle), Style::default().fg(Color::White)),
        Span::raw("    "),
        Span::styled("Nom: ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("{:.1} kV", bus.nom_voltage), Style::default().fg(Color::White)),
    ]));

    // Generators on this bus
    let gens: Vec<_> = net.generators.iter().filter(|g| g.gen_bus_id == bus.bus_id).collect();
    if !gens.is_empty() {
        lines.push(Line::raw(""));
        ...
        for g in &gens {
            ...
            lines.push(Line::from(vec![
                Span::styled(format!("  {} ", g.gen_name), Style::default().fg(Color::White)),
                Span::styled(format!("P={:.1} MW  Q={:.1} MVAR  Vset={:.4}  [{}]", g.p_gen, g.q_gen, g.v_setpoint, st),
    ...
    // Loads on this bus
    let loads: Vec<_> = net.loads.iter().filter(|l| l.bus_id == bus.bus_id).collect();
    if !loads.is_empty() {
        lines.push(Line::raw(""));
        lines.push(Line::styled(
        ...
        for l in &loads {
            lines.push(Line::from(vec![
                Span::styled(format!("  {} ", l.load_name), Style::default().fg(Color::White)),
                Span::styled(format!("P={:.1} MW  Q={:.1} MVAR", l.real_load, l.imag_load),
                    Style::default().fg(Color::DarkGray)),
            ]));
        }
    }
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(format!("{:.1} MW", p_net),
            Style::default().fg(if p_net >= 0.0 { Color::Green } else { Color::Red })),
        Span::styled(format!("  (Gen={:.1}  Load={:.1})", p_gen, p_load),
            Style::default().fg(Color::DarkGray)),
    ]));

    // Mismatch
    let (p_mis, q_mis) = net.bus_mismatch(bus.bus_id);
    let p_color = if p_mis.abs() < 0.1 { Color::Green } else { Color::Red };
    let q_color = if q_mis.abs() < 0.1 { Color::Green } else { Color::Red };
    lines.push(Line::from(vec![
    ...
    // Connected branches
    let branches: Vec<_> = net.branches.iter()
    ...
    if !branches.is_empty() {
        ...
        lines.push(Line::from(vec![
            Span::styled("  To       Type  Flow (MW)    X          Rate     Status", Style::default().fg(Color::Yellow)),
        ]));
        for br in &branches {
            let other = if br.from_bus == bus.bus_id { br.to_bus } else { br.from_bus };
            // Show flow direction: positive = into this bus convention depends on from/to
            let flow_display = if br.from_bus == bus.bus_id { br.flow } else { -br.flow };
            let st = if br.branch_status { "In" } else { "Out" };
            let flow_color = if !br.branch_status { Color::DarkGray }
                else if br.operating_limit > 0.0 && br.flow.abs() > br.operating_limit { Color::Red }
                else { Color::White };
            lines.push(Line::from(vec![
                Span::styled(format!("  {:<7}  {:<4}  ", other, br.branch_type), Style::default().fg(Color::White)),
                Span::styled(format!("{:>9.1}    ", flow_display), Style::default().fg(flow_color)),
                Span::styled(format!("{:>9.6}  {:>7.1}    {}", br.reactance, br.operating_limit, st),
                    Style::default().fg(Color::DarkGray)),
            ]));
        }
    }
